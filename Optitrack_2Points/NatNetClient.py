...
class NatNetClient:
    def __init__( self ):
        ...
        self.serverIPAddress = configParser.get('natnetConfig', 'natnetIP')
...
#        print(self.multicastAddress)
        ...
        # NatNet Data channel     
        self.dataPort = int(configParser.get('natnetConfig', 'dPort'))
    ...
    def __unpackMocapData( self, data ):
        ...
        for i in range( 0, markerSetCount ):
            ...
            inMarkerModelName.append(modelName.decode('utf-8'))
            ...
            for j in range( 0, markerCount ):
                ...
                oneMarker = {}
                oneMarker['ID'] = j
                oneMarker['pos[0]'] = pos[0]
                oneMarker['pos[1]'] = pos[1]
                oneMarker['pos[2]'] = pos[2]
                #trace( "\tMarker", j, ":", pos[0],",", pos[1],",", pos[2] )
                allMarkers.append(oneMarker)
            inMarkerset.append(allMarkers)
...
#            trace( "\tUnlabled Marker", i, ":", u_unlabeled[i][0],",", u_unlabeled[i][1],",", u_unlabeled[i][2] )
        ...
        skeletonCount = 0
        # print(self.__natNetStreamVersion[0])
        if( ( int(self.__natNetStreamVersion[0]) == 2 and int(self.__natNetStreamVersion[1]) > 0 ) or int(self.__natNetStreamVersion[0]) > 2 ):
        ...
        if( ( int(self.__natNetStreamVersion[0]) == 2 and int(self.__natNetStreamVersion[1]) > 3 ) or int(self.__natNetStreamVersion[0]) > 2 ):
        ...
        if self.newFrameListener is not None:
            self.newFrameListener( inMarkerModelName, inMarkerset, markerCount, frameNumber, markerSetCount, unlabeledMarkersCount, rigidBodyCount, skeletonCount,
